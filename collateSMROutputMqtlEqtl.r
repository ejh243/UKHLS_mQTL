...
## function to separate genic annotation category for DNAm sites
...
## Summarise number of times each feature tested
...
write.csv(signifPairs,"SupplementaryTable7.csv")

## Summarise number of times each feature was significantly associated
table(table(signifPairs$each))
table(table(signifPairs$ProbeID))
par(mfrow = c(1,3))
hist(table(signifPairs$each), xlab = "Number of times DNAm site associated")
hist(table(signifPairs$ProbeID), xlab = "Number of times transcript associated")
hist(table(signifPairs$Gene), xlab = "Number of times gene associated")
...
## Supp Fig 16
vioplot(vioData.abs[[1]], vioData.abs[[2]], vioData.abs[[3]], vioData.abs[[4]], vioData.abs[[5]], vioData.abs[[6]], 
	vioData.abs[[7]], vioData.abs[[8]], names = c(geneFeatures, "Intergenic"))
